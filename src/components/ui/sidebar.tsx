...
>(
  (
    ...
    const finalOpen = openProp ?? open
    const finalSetOpen = setOpenProp ?? setOpenWithCookie

    // Helper to toggle the sidebar.
    const toggleSidebar = React.useCallback(() => {
      return isMobile
        ? setOpenMobile((open) => !open)
        : finalSetOpen((open) => !open)
    }, [isMobile, finalSetOpen, setOpenMobile])
  ...
  (
    ...
    return (
      <>
        ...
        <div
          ref={ref}
          className={cn("group peer hidden text-sidebar-foreground", { "md:block": !isMobile })}
          data-state={state}
        ...
        >
          ...
          <div
            className={cn(
              ...
              variant === "floating" || variant === "inset"
                ? "w-[calc(var(--sidebar-width)_+_theme(spacing.4))] p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)]"
