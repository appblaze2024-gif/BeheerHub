...
export default function OpenIssuesPage() {
  ...
  const openStatuses = ["Nieuw", "Intern doorgezet", "In behandeling", "Gepland op korte termijn", "Gepland op langere termijn"];

  const filteredMeldingen = React.useMemo(() => {
    if (!meldingen) return [];
    
    let openMeldingen = meldingen.filter(m => openStatuses.includes(m.status));
  ...
  if (!selectedMelding) {
    return (
        <div className="flex flex-col flex-1 p-6">
            <h1 className="text-xl font-bold mb-4">Openstaande Meldingen</h1>
            <Card className="flex-1 flex items-center justify-center">
                <CardContent className="text-center p-6">
                    <Bell className="mx-auto h-12 w-12 text-muted-foreground" />
                    <p className="mt-4 text-lg font-semibold">Geen openstaande meldingen gevonden</p>
                    <p className="text-muted-foreground">Er zijn geen openstaande meldingen die overeenkomen met de huidige filters.</p>
                </CardContent>
            </Card>
        </div>
    )
            ...
            <div className="flex items-center gap-4">
                 ...
                 <Select value={selectedMeldingId || ''} onValueChange={setSelectedMeldingId}>
                    <SelectTrigger className="w-72">
                      <SelectValue>
                        {`Openstaande Meldingen (${filteredMeldingen.length})`}
                      </SelectValue>
                    </SelectTrigger>
                    <SelectContent>
