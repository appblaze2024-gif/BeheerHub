    ...
    <div className="flex flex-col flex-1 h-full bg-slate-50 overflow-hidden">
      ...
      >
        {selectedProjectId && (
          <div className="flex items-center gap-2">
            <Button onClick={() => setIsContractorDialogOpen(true)} className="font-black h-10 uppercase tracking-tight rounded-none">
      ...
      {!selectedProjectId ? (
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0 p-4 md:p-6 overflow-hidden">
          {/* Aannemers Lijst */}
          <Card className={cn(
            "lg:col-span-4 flex flex-col rounded-none overflow-hidden border-none shadow-xl",
              ...
              {filteredContractors.length > 0 ? (
                filteredContractors.map(c => (
                  <div 
                    ...
                    className={cn(
                      "p-4 rounded-none cursor-pointer transition-all flex items-center justify-between group",
          ...
          {/* Notulen Overzicht */}
          <Card className={cn(
            "lg:col-span-8 flex flex-col rounded-none overflow-hidden border-none shadow-xl bg-white",
            isMobile && !selectedContractor ? "hidden" : "flex"
          )}>
            {selectedContractor ? (
              <div className="flex flex-col h-full">
                <div className="p-6 border-b bg-slate-50/50 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    {isMobile && <Button variant="ghost" size="icon" onClick={() => setSelectedContractor(null)} className="rounded-none"><ArrowLeft className="h-5 w-5" /></Button>}
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    <Button variant="outline" onClick={() => setIsTemplateDialogOpen(true)} className="font-bold h-9 border-slate-300 rounded-none">
                      <Settings2 className="mr-2 h-4 w-4" /> Sjabloon
                    </Button>
                    <Button onClick={() => setIsMinuteDialogOpen(true)} size="sm" className="h-9 font-black uppercase tracking-tight shadow-lg shadow-primary/20 rounded-none">
                ...
                <div className="flex-1 overflow-y-auto p-6 no-scrollbar">
                  {isLoadingMinutes ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {minutes.map(m => (
                        <div key={m.id} className="group p-5 rounded-none border-2 border-slate-50 hover:border-primary/20 hover:bg-slate-50/30 transition-all cursor-pointer relative">
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="text-right">
                              ...
                              <Badge variant="outline" className="mt-1 h-5 text-[8px] font-black uppercase border-2 rounded-none">Verslag</Badge>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                            <div className="flex items-center gap-2">
                              <div className="h-6 w-6 rounded-none bg-slate-900 flex items-center justify-center text-[8px] font-black text-white uppercase">
                                {m.createdBy?.substring(0, 2) || 'BH'}
                              </div>
                              <span className="text-[10px] font-black text-slate-400 uppercase">{m.createdBy}</span>
                            </div>
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-300 hover:text-primary rounded-none" onClick={(e) => { e.stopPropagation(); setEditingMinute(m); setIsMinuteDialogOpen(true); }}>
                                <Pencil className="h-4 w-4" />
                              </Button>
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-300 hover:text-red-600 rounded-none" onClick={(e) => { e.stopPropagation(); handleDeleteMinute(m.id); }}>
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-slate-300">
                      <div className="bg-slate-50 p-8 rounded-none mb-4">
